...
Task ID: session13
Agent: main (Super Z)
Task: Reduce Vercel Fluid Active CPU across the whole site, with three explicit constraints from the user: (1) cut CPU from the cron-job notification chain, (2) put the bias bar + NEUTRALWIRE banner "in an image", (3) do it all WITHOUT changing the notification/image experience one bit. Full audit of every CPU source + minimization pass.

Work Log:
- ENVIRONMENT was fresh: re-cloned repo (HEAD b3c0807), bun install, dev server.
- AUDIT (every invocation source mapped): cron chain = /api/push/trigger-tz (cron-job.org, every 30 min) reading ALL devices (829 records), self-fetching /api/news × 7 (7 function invocations per notifying run!), webpush crypto per device, fire-and-forget /api/summary POST per notification (full invocation + AI call even when the summary already exists), /api/og-image render per displayed notification (full-canvas 1200×630 librsvg rasterization + 11 letter paths + digit paths per render); /api/cron/refresh-all (inherent — keeps news fresh, already parallel + after()); per-visitor: /api/referral/track on EVERY page view (full device read-modify-write in Firebase!), immediate /api/session tz ping per reload, /api/push/subscribe full device read+write on EVERY PWA launch, /api/notifications {enabled:true} boot POST per launch, /api/pwa-installed POST per launch, /api/flags no-store (function per page load), /api/top-news (17 Firebase reads/hit, no cache header). Confirmed SAFE/already-optimal: fb-stats poll (debug-gated), SW schedule polling (dormant — no START_SCHEDULE_POLL sender), SW 4s HEAD loop (offline-recovery only, fires once when back online), /api/img + /api/news + /api/topic/[id] (CDN-cached), title-rewrites + VLM verdicts (Firebase-persisted).
- OG IMAGE (the user's "in an image" ask):
  - NEW src/app/api/og-image/overlay-geometry.ts — SINGLE SOURCE OF TRUTH for all overlay geometry: canvas, bar bounds, banner bounds (BANNER_REGION = the exact integer device rect covering shadow+pill+letters), buildBannerLayerSvg() (full-canvas transparent, build-time only), buildBiasBarRegionSvg() (the bar as a TINY 1160×52 region SVG — every element translated by the integer vector (−20,−558) so shapes land on identical device pixels), buildFallbackSvg().
  - NEW scripts/gen-og-overlays.ts (bun): renders the banner ONCE → 421×81 lossless transparent PNG (4,807 bytes) + the fallback OG image ONCE through the exact old pipeline (sharp(svg).jpeg({quality:85}), no mozjpeg) → 9,786-byte JPEG; writes both base64 into NEW src/app/api/og-image/overlay-assets.ts (bundled with the route — zero fs, zero runtime rasterization for static layers).
  - route.ts rewritten: composites [tiny bar-region SVG at (20,558)] + [pre-baked banner PNG at (760,466)] over the resized base — bar+banner are now IMAGE composites, not per-request vector renders. Fallback responses serve the pre-baked JPEG verbatim (zero sharp work). Regions are provably non-overlapping (banner rows 466–546 vs bar rows 558–610).
  - NEW scripts/test-og-pixels.mjs: reconstructs the OLD pipeline VERBATIM and diffs old vs new for 3 bases × 7 bias combos + fallback + banner: 23/23 BYTE-IDENTICAL JPEGs — under bun AND node (Vercel runs node; the dev-server output was also confirmed byte-identical to the node-rendered new pipeline). The image does not change one bit, proven.
- TRIGGER-TZ (the user's cron CPU ask):
  - The 7 × /api/news self-fetches are GONE: the route now reads the SAME Firebase newsCache nodes directly (readCachedNews) and applies the EXACT filter /api/news applied (coverage≥1, slice 0–5). Stale-cache semantics identical (/api/news returned stale instantly too); missing-node edge case falls back to the HTTP route per category (preserves first-ever-run behaviour). ~7 function invocations (each with cold-start module load) eliminated per notifying cron run.
  - Summary pregen pre-check: before firing the /api/summary invocation, reads summaries/<topicId> and skips the POST when a REAL (non-template) summary exists (mirrors isTemplateSummary's 3 markers). Still fire-and-forget (wrapped in a void async IIFE — never delays the push loop).
- CLIENT CALL-GATES (per-visitor invocations, identical semantics):
  - NEW src/lib/client-call-gate.ts: localStorage gates, fail-OPEN (broken storage → call always fires); a gate skips a call only when its semantic fields are unchanged AND the heartbeat TTL hasn't lapsed; marks are written only after the server confirms success.
  - page-client.tsx wiring: /api/referral/track gated to 24h heartbeat (NO ?ref= loads only — referral attribution NEVER gated); immediate /api/session tz ping skipped when the same tz was pinged <4.5 min ago (marks updated by the 5-min pings too); /api/push/subscribe skipped when endpoint+standalone unchanged within 24h (new/rotated subscriptions and first-launch-after-install always post); /api/notifications {enabled:true} boot POST once-ever (the permission-grant flow + every settings toggle remain ungated); /api/pwa-installed 24h heartbeat (the appinstalled event always reports + re-arms the gate).
- CDN EDGE CACHING: /api/flags no-store → s-maxage=60 + swr=120 (the design the user SEES still comes from page.tsx SSR Firebase reads — flips stay live on reload); /api/summary GET found-summaries → s-maxage=300 + swr=600 (404s uncached so later-generated summaries are found); /api/top-news → s-maxage=120 + swr=300 (same freshness class as the already-cached /api/news).
- VERIFIED (scripts/test-session13.sh, 11/11): pixel identity 23/23 byte-identical under bun + node; dev-server og-image override render 200 image/jpeg; fallback served byte-identical to the pre-baked asset; trigger-tz dry run ok (829 devices scanned, no self-fetch); flags/top-news CDN headers live; all 5 gate keys compiled into the client bundle; agent-browser: first visit fires referral/track + session ping with markers written, reload fires ZERO of them, cleared-storage visit fires them again, feed renders (53 headings), zero console/page errors. tsc 0 errors (baseline), eslint clean.

Stage Summary:
- Cron notification chain: ~7 function invocations + redundant summary invocations per notifying run eliminated — the notification content, timing, personalization and the image the OS shows are unchanged (byte-identical image, same story selection, same slots).
...
## Session 15 — Watch button: article-only + bottom-left + video quality requirements

Task ID: 15
Agent: main (Super Z)
Task: Refine the experimental Watch feature per user: (1) NO Watch button on the home screen — only when an article is opened, (2) positioned on the LEFT side of the image (not right), (3) videos must come from channels with at least 10k subscribers and run more than 10 seconds.

Work Log:
- Workspace was fresh (state 17237be = session14 worklog). Environment rebuilt: dev server on :3000, no build runs.
- UI — WatchPill removed from ALL feed cards (topic-card.tsx: mini thumbnail pill, hero/default ImageBadges pill, the VideoPlayer overlay instance, useVideoWatch/videoOpen state and imports; ImageBadges reverted to the plain NW watermark). WatchPill lives ONLY in topic-detail's hero now: NW brand mark keeps bottom-RIGHT, the pill owns bottom-LEFT (its own absolute bottom-2 left-2 positioning; compact variant + cn import deleted). Context flag plumbing (video-watch.tsx provider) unchanged — page.tsx still feeds it SSR.
- API — quality gate extracted to NEW src/lib/video-quality.ts (unit-testable pure parsers + the gate): searchYouTubeForStory() now parses ytInitialData properly (brace-walking JSON extractor, videoRenderer walk → videoId/title/author/channelUrl/duration), prefers candidates whose channel matches the story's own outlets (normalized containment — the "source's video" preference), then relevance order; checkYouTubeVideo() enforces duration > 10s (search lengthText — MANDATORY hint), oEmbed liveness/embeddability, and >= 10k channel subscribers (channel page scrape, 3 pattern shapes). KEY DISCOVERY: youtube.com/watch pages 429-block datacenter IPs (Google sorry page) — verified live; embed pages carry no duration, m/shorts also 429, channel RSS has no durations — so the search result's lengthText is the ONLY keyless duration source, and a bare videoId (unverifiable) is always rejected. The old direct-embed of YouTube-shaped RSS videoUrls/links was removed for exactly that reason; native mp4/m3u8 enclosures still play directly, gated on the feed's declared media:content duration. Cache namespace bumped videos/ → videos2/ so every pre-requirements cached result is ignored. A ~10.5s resolution budget bounds all fetches (maxDuration 15).
- FIXED a real bug found by unit tests: the ytInitialData assignment regex required a literal quote (only matched window["ytInitialData"] = {, never var ytInitialData = {) — now ["'\]]* and both shapes unit-tested; before the fix the search fell to a metadata-less regex path and returned null.
- RSS — news-aggregator extractVideoAttr → extractVideoMedia: also captures the MRSS media:content duration attribute (seconds) as FeedArticle.videoDuration (additive; old cache entries validate).
- /debug — videoWatch card description updated (article-only, bottom-left, >10s + >=10k subs requirements).
- VERIFIED (scripts/test-video-requirements.ts, 74/74): 30 parser unit tests (subscriber shapes, clocks, brace-walking incl. braces-in-strings + both assignment shapes, renderer candidates, URL shapes); live: BBC News channel parses 20M subs, searchYouTubeForStory returns a Nepal-floods BBC video that INDEPENDENTLY re-verifies (>10s, alive, 20M subs), bare videoId rejected; e2e via dev server: 8/8 real feed topics resolved to qualifying videos (ABC 19.8M subs/105s, FRANCE 24 8.68M/195s, Fox Business 15.5M/791s, Al Jazeera 23.5M/84s, Newsmax 2.59M/233s, BBC 20M/357s …) each independently re-verified from fresh search + channel scrapes; videos2/ cache identical on 2nd call + persisted in Firebase. scripts/test-video-flag.ts (5/5): flag off via Firebase → /api/video returns reason 'disabled' (after memo expiry) → flag on resolves again. Browser (agent-browser): home feed 0 watch pills; article opens with the pill at bottom-LEFT (pill.left 280 vs image.left 272; brand mark at right 868-1000 vs image.right 1008); click → VideoPlayer overlay with youtube-nocookie embed of a matching news video; flag off → reload → article shows 0 pills; flag restored; zero console/page errors. tsc 0 errors (the one TS18047 that appeared mid-session was a stale tsbuildinfo artifact — clean on re-run), eslint clean.
- Note: the flag-memo (60s) means a /debug flip takes up to ~60s to reach a warm /api/video instance (page UI flips on next load as before — SSR read).

Stage Summary:
- Watch button now exists ONLY inside the opened article, pinned to the image's bottom-left corner (brand mark keeps bottom-right); home screen is pill-free.
- Every video that plays is: about the story (relevance or the story's own outlet), longer than 10 seconds, from a channel with at least 10k subscribers, and embeddable — proven live with independent re-verification across 8 real stories + unit tests.
- Watch pages are dead from server IPs — the resolver now relies solely on the three surfaces that work (search results, oEmbed, channel pages) and rejects anything it cannot verify. Kill switch unchanged: one click in /debug removes pill + resolution CPU.

---
Task ID: 15
Agent: main (Super Z)
Task: 5 watch/video fixes — dark-mode white line at top of mobile screen, adaptive video popup proportions, auto-retry failed fetches, black square Watch button, remove "experimental" copy

Work Log:
- White line: reproduced in emulated iPhone 14 dark mode — a 1px #232324 strip at y=0. Root cause: `.platform-apple/.platform-android .glass` rules painted `border: 1px solid rgb(255 255 255 / 0.08-0.12)` on the FULL-WIDTH sticky header (page header, user-page, topic-detail bars); the top border hairline is invisible in light mode, glaring in dark. Fix: removed `border:` from all `.platform-* .glass` + `.glass-liquid` variants in globals.css (sticky bars keep their own border-b; pwa-onboarding cards keep their own border classes). Verified: top rows now uniform (16,16,18).
- Adaptive popup: measured the API-reported ratio server-side — YouTube's oar2.jpg / oardefault.jpg ORIGINAL-ASPECT thumbnails (standard thumbs are always padded 16:9; oar2 returns true dims: verified 0.5625 for vertical, 1.778 for landscape, 404 for some videos). Added jpegDimensions() (SOF-marker parser on a 16KB Range fetch) + fetchYouTubeAspectRatio() + withAspect() in video-quality.ts; QualifiedVideo/VideoResult gained `aspect`; route passes it through and caches it. Client: video-player.tsx sizes BOTH the card and video box — `width: min(100%, calc(62dvh * ratio + 2px), 42rem)` + `aspect-ratio` + maxHeight 62dvh; native <video> self-measures via onLoadedMetadata (videoWidth/videoHeight). Verified live: the CNBC-TV18 vertical video (aspect 0.5625 — the exact "mobile proportions" case the user hit) now renders a 296x639 portrait popup with a 294x523 9:16 box; landscape stays 358x316; desktop 672x476 centered. Videos without oar thumbs fall back to 16:9.
- Auto-retry: client re-runs a failed fetch up to 2x (700ms apart) with `?retry=N`; "disabled" is never retried. Server: retry runs skip CACHED MISSES (re-resolve live) but still short-circuit cached FOUND. Verified end-to-end: injected a fake miss → plain GET returned it instantly (1.3s) → UI auto-fired ?retry=1 → server re-resolved (5.7s) → found the video → popup played it. Retrying state shows "First attempt came up empty — trying again…".
- Watch button: redesigned WatchPill — clean black squircle (54x48, bg-black/90, rounded-xl) embedded bottom-left of the hero image, large drawn white play triangle (custom SVG 19px) + small uppercase "WATCH" text beneath. VLM-verified appearance. No watch button on home cards (unchanged from previous commit).
- Copy: removed user-facing "experimental" strings ("Watch — experimental" → "No video found for this story"; footer "Experimental — plays…" → "Plays a news video about this story"). Remaining mentions are code comments / password-gated /debug only.
- Tailwind dev-server gotcha: rewritten files can miss the JIT class scan (new arbitrary classes like pt-[10px] not compiled) — fixed by appending a content-changing trigger, then removed it.
- Verification: lint 0 errors, tsc 0 errors (baseline), browser tests on mobile dark + desktop, scripts/test-aspect.ts (oar2 landscape/portrait/bogus cases pass). Committed f02ecf0, pushed to main.
...
Work Log:
- Offline false positive (sw.js): root-caused to the v23 2.5s navigation race. A fresh load with no cached HTML (first visit / 12h-swept shell cache) that lost the race fell straight to the "Waiting for connection…" offline page — even though the connection was fine, just slow (this page is exactly the message the user quoted). Fix: the 2.5s race is now only the FAST path — when cached HTML exists nothing changes (cache serves instantly, the slow fetch refreshes it in the background); when NO cache exists, we keep awaiting the ORIGINAL in-flight fetch up to a 20s hard timeout before declaring offline. A genuinely offline fetch rejects in milliseconds, so the real offline page still appears fast; a slow-but-alive first load now lands the app. Offline page recovery poll 4s → 2.5s. Cache names bumped v25 → v26 (shell/api/img) so installed PWAs pick up the new logic on next launch (registration uses updateViaCache:'none').
- Preview sound button (video-preview-store.ts + video-preview.tsx): the old bottom-left "Preview" chip was passive (pointer-events-none, icon-only). It is now a real pressable Sound button, rendered ONLY while the preview is playing (user spec). The press IS the user gesture the autoplay policy requires (user: "only where there is a press registered on a website it can do sound"), so un-muting from it reliably sticks. Plumbing: PreviewControls gained optional setAudible(on); the store gained setPreviewAudible(topicId,on) + forcePreviewAudio(topicId) — a manual press outranks automatic lease grants: the lease is forced to the pressed preview and the previous audible preview is muted through its registered controls (two previews never talk over each other). Both players implement setAudible: YT (unMute+setVolume(50) / mute, reusing unMuteNow) and native <video> (muted=false+volume=0.5 / mute, with a muted retry if the policy still refuses). Muting via the button parks the lease on the topic so no queued preview suddenly makes noise the user just asked to stop. Tapping the button never opens the article: pointer-events-auto over the pass-through container + pointerdown/click stopPropagation + the card's closest('a,button') guard.
- Gesture-race guard: the press's own pointerdown fires the document-level gesture-recovery (capture listener), which can flip `muted` BETWEEN pointerdown and click — the click would then read the flipped state and toggle sound straight back off (opposite of intent). The button captures what it DISPLAYED at press time (pressedMutedRef) and the click applies that intent deterministically.
- Chrome never auto-reveals (video-player.tsx): root cause — VideoChrome's effect auto-showed the bar whenever `playing` was false, i.e. from mount (player paused while autoplay spun up) until 3s after playback actually started: exactly the ~2s of progress-bar + pause/play + volume clutter over a freshly loaded video. Removed the effect entirely: the bar now appears ONLY from a user tap (or desktop mousemove). Reveal call sites re-tuned around the PRE-toggle `playing` prop: video-surface tap and bar play/pause → show(!playing) (a tap that PAUSES keeps the bar up — user mid-decision; a tap that resumes auto-hides 3s later); mute/fullscreen buttons and mousemove → show(playing) (play state unchanged). The centered play affordance is untouched (the player area is opacity-gated until first PLAYING, so it never flashes at load — only after a pause).
- Verified: bunx tsc --noEmit 0 errors (baseline), eslint clean on all three changed files, node --check on sw.js, dev server 200 on / and /sw.js with the v26 constants served, no errors in the dev log.
- Committed 6d76c3e, pushed to main.

Stage Summary:
- Slow fresh loads no longer misreport offline (2.5s race is a fast path; no-cache navigations wait up to 20s for the real fetch before showing the offline page).
- The preview now has a real Sound button (bottom-left, only while playing) that leverages the press-as-gesture rule, with audio-lease override so pressing one preview's sound mutes any other audible preview.
- The article video loads completely clean — the control bar only ever appears when the user taps the video (or hovers on desktop).
