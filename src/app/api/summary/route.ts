...
Write a neutral, in-depth summary of this story following the rules above.`
  ...
  if (summary && !/[.!?]$/.test(summary.trim())) {
    try {
      const contCompletion = await zai.chat.completions.create({
        messages: [
          { role: 'assistant', content: systemPrompt },
          { role: 'user', content: userPrompt },
          { role: 'assistant', content: summary },
          { role: 'user', content: 'Continue and complete the summary. Do not repeat what you already wrote.' },
        ],
        thinking: { type: 'disabled' },
        max_tokens: 300,
      })
      const continuation = contCompletion.choices[0]?.message?.content?.trim()
      if (continuation) {
