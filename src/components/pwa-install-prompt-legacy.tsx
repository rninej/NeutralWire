...
import {
  ensureFirstSeen,
  endFirstVisitSession,
  isFirstVisitSession,
} from '@/lib/popup-mode'

const DISMISS_KEY = 'neutralwire:pwa-install-dismissed'
// The 'first-visit' variant (smart-firstvisit mode) writes its dismissal to
// a SEPARATE key so a first-visit "Not now" never snoozes the smart
// install engine's long-term behavioral memory (that engine reads
// DISMISS_KEY with 3-day snooze semantics — see pwa-install-prompt.tsx).
const DISMISS_KEY_FV = 'neutralwire:pwa-install-fv-dismissed'
...
/**
 * PwaInstallPromptLegacy — the ORIGINAL install popup, restored verbatim
 * from before the behavioral rewrite, selectable from /debug (popup
 * system = 'original' or 'smart-firstvisit').
 *
 *   variant 'original'     — exact classic behavior: arms on every visit,
 *                            1-hour re-ask, original localStorage keys.
 *   variant 'first-visit'  — arms ONLY inside a brand-new visitor's very
 *                            first visit (the classic high-visibility
 *                            popup), with isolated ':fv' dismiss keys so
 *                            the smart engine (which owns visits 2+) keeps
 *                            a pristine behavioral memory.
 *
 * Everything below — triggers, guards, browser detection, JSX — is the
 ...
 */
export function PwaInstallPromptLegacy({
  variant = 'original',
}: {
  variant?: 'original' | 'first-visit'
} = {}) {
  ...
  // Variant captured in a ref so the mount-only effect below can read it
  // without re-subscribing (the prop never changes mid-mount in practice).
  const variantRef = React.useRef(variant)
  // The dismiss key this variant writes (see DISMISS_KEY_FV above).
  const dismissKey = variant === 'first-visit' ? DISMISS_KEY_FV : DISMISS_KEY
  ...
  React.useEffect(() => {
    // ── Arming gate ──
    // Create/refresh the first-visit tracking first (idempotent), then:
    // the 'first-visit' variant only arms while the current tab is still
    // inside the visitor's very FIRST visit. Any later visit → dormant.
    ensureFirstSeen()
    if (variantRef.current === 'first-visit' && !isFirstVisitSession()) {
      return
    }
    ...
    // Returns true if the user recently dismissed the prompt (within 1h).
    const isDismissed = () => {
      const dismissedAt = localStorage.getItem(
        variantRef.current === 'first-visit' ? DISMISS_KEY_FV : DISMISS_KEY,
      )
    ...
    const installedHandler = () => {
      ...
      installedRef.current = true
      if (variantRef.current === 'first-visit') endFirstVisitSession()
      setShowBanner(false)
  ...
  const handleDismiss = () => {
    // Set the 1-hour dismiss cooldown (variant-scoped key — see
    // DISMISS_KEY_FV). The popup will re-appear after 1 hour OR when the
    // site is reopened (the dismiss timestamp is checked on mount against
    // the current time).
  ...
  const handleNativeInstall = async () => {
    ...
    try {
      ...
      if (choice.outcome === 'accepted') {
        ...
        // User dismissed the native prompt — set the 1h cooldown.
        localStorage.setItem(dismissKey, String(Date.now()))
      }
      setShowBanner(false)
      setDeferredPrompt(null)
